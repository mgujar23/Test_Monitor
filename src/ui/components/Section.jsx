...
export default function Section({ title, data, sectionKey, onClickMetric }) {
  ...
  if (sectionKey === 'readyCluster') {
    return (
      <div className="bg-dark-card border border-dark-border rounded-lg p-4 mb-4">
        <h2 className="text-xl font-bold text-white mb-4">{title}</h2>

        {!data || !data.builds || data.builds.length === 0 ? (
          <div className="text-gray-400 text-sm">No build data available</div>
        ) : (
          <>
...
{currentDiff}
  ...
  // ========== REPORTING TESTS, PROXY STATUS, AND SIMILAR SECTIONS (with builds) ==========
  if (['reportingTests', 'proxyStatus', 'proxyLogs'].includes(sectionKey) && data?.builds) {
    return (
      <div className="bg-dark-card border border-dark-border rounded-lg p-4 mb-4">
        <h2 className="text-xl font-bold text-white mb-4">{title}</h2>
          ...
          <>
            ...
            {data.changes && data.changes.length > 0 && (
              <div className="mt-6 bg-dark-bg rounded border border-dark-border">
                ...
                {isChangesExpanded && data.changes.length > 0 && (
                  <div className="border-t border-dark-border p-4">
                    <div className="overflow-x-auto border border-dark-border rounded" style={{ maxHeight: '500px' }}>
                      <table className="w-full text-xs text-gray-300">
                        <thead className="bg-dark-card border-b border-dark-border sticky top-0">
    ...
    return (
      <>
        <div className="bg-dark-card border border-dark-border rounded-lg p-4 mb-4">
          ...
          {/* Portal Links */}
          {sectionKey === 'csgServiceReporting' && (
            <div className="flex justify-end mb-4">
              <a
                href="https://jenkins.cicd.cloud.fpdev.io/job/GHE-CSG-DEV/job/csg_service-reporting/job/master/"
                target="_blank"
                rel="noopener noreferrer"
                className="text-blue-400 hover:text-blue-300 text-xs underline"
              >
          ...
          {sectionKey === 'cstoreReporting' && (
            <div className="flex justify-end mb-4">
              <a
                href="https://jenkins.infra-dev.forcepoint.net/job/Projects/job/Test/job/CStoreReportingTest/"
                target="_blank"
                rel="noopener noreferrer"
                className="text-blue-400 hover:text-blue-300 text-xs underline"
              >
          ...
          {sectionKey === 'etlSIEM' && (
            <div className="flex justify-end mb-4">
              <a
                href="https://jenkins.cicd.cloud.fpdev.io/job/GHE-CSG-DEV/job/etl-siem/job/master/"
                target="_blank"
                rel="noopener noreferrer"
                className="text-blue-400 hover:text-blue-300 text-xs underline"
              >
          ...
          {/* Status Message for Failed Builds */}
          {data.statusMessage && (
            <div className="mb-4 p-3 bg-yellow-900/30 border border-yellow-600/50 rounded text-yellow-200 text-sm">
              {data.statusMessage}
            </div>
          )}
          ...
          <div className="grid grid-cols-4 gap-3 mb-4">
            {[
              { key: 'total', label: 'Total tests', value: data.total || 0 },
              { key: 'failed', label: 'Failed tests', value: data.failed || 0 },
              { key: 'passed', label: 'Passed tests', value: passed },
              { key: 'stale', label: 'Stale tests', value: data.stale || 0 }
            ].map((metric) => (
              <button
                key={metric.key}
                  ...
                  <div className="mt-4 bg-dark-bg rounded border border-dark-border overflow-hidden">
                    ...
                    >
                      ...
                      <div className="text-sm font-semibold text-gray-300">
                        ...
                        {selectedMetric === 'stale' && 'Stale Test Areas'}
                        {selectedMetric === 'passed' && 'Passing Tests by Area'}
                        {selectedMetric === 'total' && 'Tests by Area'}
                      </div>
                    </button>
                    ...
                    {selectedArea !== 'collapsed' && (
                      <div className="p-4 border-t border-dark-border">
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                          {areasArray.map((area, idx) => {
                            const shouldShow = selectedMetric === 'failed' ? area.failed > 0 : true;
                            if (!shouldShow) return null;
                    ...
                    <table className="w-full text-xs text-gray-300">
                      ...
                      <tbody>
                        {visibleTests.length > 0 ? (
                          ...
                          <tr>
                            <td colSpan="4" className="px-3 py-4 text-center text-gray-500">No tests found</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
  ...
  if (['reportingMetrics'].includes(sectionKey)) {
    return (
      <div className="bg-dark-card border border-dark-border rounded-lg p-4 mb-4">
        <h2 className="text-xl font-bold text-white mb-4">{title}</h2>
        <div className="text-gray-400 text-sm">No data available - configure monitoring source</div>
      </div>
    );
  }
